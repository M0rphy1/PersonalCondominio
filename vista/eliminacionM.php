...
if(isset($_GET['cedula'])){
    ...
    $sql = "SELECT p.codigo AS codigo_personal, m.codigo AS codigo_limpieza, p.cedula, p.nombre, p.apellido, m.nombrearea, m.sueldo 
            FROM personal p 
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            ...
            echo "<td>
                    <form method='post' action='eliminarpM.php'>
                        <input type='hidden' name='codigo_personal' value='" . $row["codigo_personal"] . "'>
                        <input type='hidden' name='codigo_mantenimiento' value='" . $row["codigo_mantenimiento"] . "'>
